...
export default function Auth() {
  ...
  // Figures out where a signed-in user should land: dashboard if their
  // signup is fully finished (profile + wedding membership), or the
  // details step if either piece is still missing — which also covers
  // someone resuming a signup that got interrupted partway through.
  const resolveDestination = useCallback(
    async (userId: string, userEmail: string) => {
      ...
      setHasProfile(!!profileRow);
      if (profileRow) setName(profileRow.name ?? '');
      ...
      const pending = await findPendingInvite(userEmail);
      setInvite(pending);
      setEmail(userEmail);
      setStep('details');
    },
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      await resolveDestination(user.id, user.email ?? '');
      setChecking(false);
    };
  ...
  const handleCredentialsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signup') {
        ...
        await resolveDestination(user.id, user.email ?? email);
      }
    } catch (err: unknown) {
  ...
  const handleDetailsSubmit = async (e: React.FormEvent) => {
    ...
    if (!invite && !hasProfile && (!weddingDate || !city || !budget)) {
  ...
  if (step === 'details') {
    return (
      <div
        className="flex items-center justify-center min-h-screen px-4 py-10 bg-cover bg-center"
        style={{ backgroundImage: "url('/images/sign-in-hero.jpeg')" }}
      >
        <div className="w-full max-w-md bg-white/95 backdrop-blur p-8 rounded-2xl shadow-lg">
          ...
          <form onSubmit={handleDetailsSubmit} className="space-y-4">
            <input
              ...
              className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#E4B441]"
              required
            />
            ...
            <div className="grid grid-cols-2 gap-3">
              <select
                ...
                className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#E4B441] text-gray-700"
              >
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#B85042] text-white py-2 rounded-lg hover:bg-[#A03F37] transition disabled:opacity-50"
            >
      ...
      <div className="w-full max-w-md bg-white/95 backdrop-blur p-8 rounded-2xl shadow-lg">
        ...
        <p className="text-center text-sm mt-4">
          ...
          <span
            ...
            className="text-[#B85042] font-semibold cursor-pointer hover:underline"
          >
